...
# Полный набор слов (свиток, выпуск, арка — СИ-специфика). "т." — тот же
# аббревиатурный маркер тома, что synchronization.py/fb2_compiler.py сами
# пишут в суффикс ("(т. N-M)", "… т. N.fb2") — баг №109 (продолжение):
# без него том без <sequence> в метаданных, единственный номер которого
# держался на "SeriesRoot N." (Правило 2), терял номер при повторном
# скане ПОСЛЕ переименования synchronization-ом в "Автор - Серия. …
# т. N.fb2" — "Серия" там больше не стоит сразу перед цифрой (мешает
# точка), а полное слово "том" в суффиксе никогда и не писалось.
_TOM_WORD_RE = re.compile(
    r'\b(?:свиток|том|т\.|книга|часть|выпуск|арка|book|vol\.?|part)\s+(\d{1,4})(?!\.\d)\b',
...
class Pass2SeriesFilename:
    ...
    def _detect_named_arcs(self, records: List[BookRecord]) -> None:
        ...
        # Реальный случай (docs/quality-roadmap.md, баг №27, часть 2 и баг
        # №31): "Галеев Эдуард - S-T-I-K-S. Сварной-1.fb2" … "…Сварной-5.fb2"
        # (дефис) и "Елисеев Алексей - S-T-I-K-S. Богиня Смерти 2.fb2" +
        # "…Богиня Смерти 2 (дополнение).fb2" (пробел + опциональная
        # аннотация, оба тома с ОДИНАКОВЫМ номером — основной том и его
        # дополнение/расширенное издание) — франшиза-обёртка "S-T-I-K-S" уже
        # извлечена как proposed_series (голый корень, без арки), но имя
        # арки вообще не распознавалось: _ARC_RE_ANY (ниже) требует "Корень
        # N. Заголовок" (точка+пробел+текст ПОСЛЕ числа) — здесь же номер
        # явно завершает имя файла (не считая опциональной аннотации), без
        # подзаголовка вовсе.
        ...
        # требуемых ≥2 вхождений, но пара "база + том N" — уже
        # самостоятельное, достаточное подтверждение дуги: баг №33).
        _BASE_ARC_RE = re.compile(r'^(?:.+?\s*-\s*)?(.+?)\.\s+(.+)$', re.UNICODE)

        for (_author_k, _series_k, _arc_k, _folder_k), entries in _dash_groups.items():
            if len(entries) < 2:
                # Ищем безномерный "базовый" файл той же дуги — вместе с
                # единственным численным томом этого достаточно для
                # подтверждения (см. комментарий выше).
                _base_rec = None
                for _r in records:
                    if (_r.proposed_series or '') != entries[0][0].proposed_series:
                        continue
                    if 'filename' not in (_r.series_source or ''):
                        continue
                    if _norm_s(_r.proposed_author or '') != _author_k:
                        continue
                    if str(Path(_r.file_path).parent) != _folder_k:
                        continue
                    if _r is entries[0][0]:
                        continue
                    _stem_b = Path(_r.file_path).stem
                    _m_b = _BASE_ARC_RE.match(_stem_b)
                    if not _m_b:
                        continue
                    if _norm_s(_m_b.group(1)) != _series_k:
                        continue
                    if _norm_s(_m_b.group(2).strip()) != _arc_k:
                        continue
                    _base_rec = _r
                    break
                if _base_rec is None:
                    continue  # уникальное совпадение — не подтверждённая дуга
                entries = entries + [(_base_rec, 1, entries[0][2])]
            # ПРИМЕЧАНИЕ: совпадающие номера у ≥2 записей (напр. основной том
            # и его "(дополнение)"/доп. издание с тем же номером) НЕ отвергаем
            # — совпадение имени арки (уже точное, по regex) само по себе
            # достаточное подтверждение; отдельные позиции тут не обязаны
            # иметь разные номера, в отличие от полноценной серии из
            # нескольких оригинальных томов.
    ...
    def _correct_series_number_from_filename(self, records: List[BookRecord]) -> None:
        ...
        Примеры:
          ...
          «01 - Варварин Остров.fb2»               → series_number=1 (пробел перед «-»)
        ...
        _PREFIX_RE = re.compile(r'^(\d{1,3})\s*[_\-–—\.]')
        # «01 Title» — вообще без знака пунктуации, только пробел перед заглавной
        # буквой/кавычкой/скобкой. Без этого правила такие файлы получали номер
        # из <sequence> в метаданных FB2 (Правило 5/_MID_NUM_RE ниже — только
        # заполнитель пробелов, не перезаписывает metadata), а тот у некоторых
        # авторов (см. цикл «История Галактики» Ливадного) внутренне противоречив
        # и не совпадает с явной хронологией имён файлов — тогда как ведущий номер
        # в имени файла для такой раскладки надёжнее, как и остальные варианты
        # префикса выше/ниже. Якорим к началу стема (^), а не ищем по всей строке
        # (как Правило 5), чтобы не подхватить случайное число в середине названия.
        _PREFIX_SPACE_RE = re.compile(r'^(\d{1,3})\s+(?=[А-ЯЁA-Z«"(])', re.UNICODE)
        ...
        for record in records:
            ...
            # Файл — компиляция всей серии (или нескольких серий) в одну книгу
            # ("Сборник" и т.п. из collection_keywords в имени/названии), но точный
            # диапазон определить не удалось (нет «(N книг)» или «(N-M)» — иначе
            # сработали бы Правила 3/3.5/3.6 выше). Ведущее число тут — не номер
            # тома внутри этой серии, а позиция файла в папке "Сборники" среди
            # РАЗНЫХ сборников автора (см. "5. Отдел 15-К. Сборник.fb2" — не
            # 5-я книга серии "Отдел 15-К", а 5-й файл в папке). Присвоение
            # такого числа как series_number ломает проверку "серия завершена?"
            # у компилятора: он видит "ещё есть том 5" и вместо "Трилогия"
            # пишет "т. 1-3". Лучше оставить номер пустым, чем ложный.
            if self.collection_keywords:
                _stem_lower = stem.lower().replace('ё', 'е')
                _title_lower = (record.file_title or '').lower().replace('ё', 'е')
                if any(kw.lower() in _stem_lower or kw.lower() in _title_lower
            ...
            m = _PREFIX_RE.match(stem) or _PREFIX_SPACE_RE.match(stem)
            if m:
            ...
            # Баг №109 (продолжение): "." тоже допустимый разделитель между
            # корнем серии и номером — "В закоулках мироздания. 9. В дни
            # Бородина.fb2" (точка сразу после корня, частый стиль этой
            # библиотеки). Без неё том без <sequence> в метаданных (единственный
            # источник номера — filename) оставался без series_number вовсе.
            m2 = re.search(
                r'(?i)' + re.escape(_sr_n)
            ...
            if 1900 <= fn_lo2 <= 2099:
                continue
            # Если ДАЛЬШЕ в имени файла есть явное ключевое слово тома
            # («Том M», «Часть M», «Книга M» и т.п.) с ДРУГИМ числом — оно
            # надёжнее общего совпадения «КореньСерии N.»: число сразу после
            # имени серии нередко оказывается номером арки/части внутри
            # серии, а не номером тома, тогда как «Том M» — однозначный
            # маркер позиции самого файла. Реальный случай: 4 разных файла
            # «Вязовский - 15 ножевых 1. Пятнадцать ножевых. Том 2/3/4/5.fb2»
            # — у всех совпадает «15 ножевых 1.» (номер части, не тома), а
            # различаются они только явным «Том N» в хвосте — но это правило
            # съедало общий «1» первым, и все 4 тома получали одинаковый
            # series_number=1. Оставляем номер незаполненным — его подхватит
            # Правило 6 (Том-keyword) ниже по стему.
            _tom_tail_m = _TOM_WORD_RE.search(_stem_n2[m2.end():])
            if _tom_tail_m and _tom_tail_m.group(1) != fn_val2.split('-')[0]:
                continue
    ...
    def _resolve_hierarchical_flat_mismatch(self, records: List[BookRecord]) -> None:
        ...
        ПРИМЕЧАНИЕ (баг №115, расследовано и отклонено): казалось бы логичным
        не пускать сюда уже папочные источники (`_FOLDER_SOURCES_ARC`) — но
        реальные данные (Бессонов Алексей/Мир Алекса Королёва/5. Миры
        Конфедерации, Эльтеррус Иар/Отзвуки серебряного ветра/7. Иная Терра)
        показали, что это ЛЕГИТИМНЫЙ сценарий: папочная 3-уровневая иерархия
        (автор\корень серии\именованная подсерия) и должна перемаркироваться
        сюда в `filename_named_arc` — это и есть механизм, которым такие
        случаи доезжают до финального CSV корректно. Настоящая причина
        бага №115 — в другом месте (`regen_csv.py:_compute_folder_series`,
        `_is_author_variant`) и починена там; трогать эту функцию не нужно.
        """
    ...
    def _extract_series_from_filename(self, file_path: str, validate: bool = True, metadata_series: str = "", known_series: str = "", proposed_author: str = "") -> str:
        ...
        # ДО очистки переводчика ниже: "…(пер. Х) - 2008" иначе не считается
        # ТРЕЙЛИНГ-скобкой (год после неё) и не вырезается вовсе.
        name_for_parsing = re.sub(r'(?:\s*[-–—])?\s*(?:19|20)\d{2}\s*$', '', name_for_parsing).strip()
        # "(пер. Имя Фамилия)" / "(перевод Имя Фамилия)" — указание переводчика,
        # не название серии. Реальный случай (docs/quality-roadmap.md, баг №55):
        # "Гришэм. Остров Камино 1. Остров Камино (пер. Виктор Антонов).fb2" —
        # без этой очистки "Правило 2" (серия в скобках в конце) ниже принимало
        # "пер. Виктор Антонов" за имя серии (точка после "пер" даже давала
        # ложное совпадение с паттерном "Серия. service_words").
        name_for_parsing = re.sub(
            r'\s*\(\s*пер(?:\.|евод)?\s+[^)]*\)\s*$', '', name_for_parsing, flags=re.IGNORECASE,
        ).strip()
        ...
        try:
            ...
            if file_patterns:
                ...
                # Корень серии — голая инициаль ("С.", "М.") ИЛИ concatенация
                # нескольких инициалов без пробелов ("С.К.С.") — никогда не
                # бывает настоящим названием серии. Реальный случай (баг №50):
                # "Битон М. С. - Хэмиш Макбет 1. Смерть сплетницы" —
                # двусоставный псевдоним автора "М. С." (M.C. Beaton)
                # сматчился по паттерну "Author. Series. SubSeries. Title" как
                # Author="Битон М." + Series="С." + SubSeries="Хэмиш Макбет 1"
                # — второй инициал автора ошибочно принят за отдельный блок
                # "Series". Второй реальный случай (баг №59): "С.К.С. -
                # Режим бога. Книга 1" — "С.К.С." (инициалы-псевдоним автора,
                # объединённые точками без пробелов) сматчился как корень
                # серии для "Режим бога" вместо того, чтобы быть отброшенным
                # как имя автора. Отбрасываем ТАКОЙ series_from_block целиком:
                # если корень (до '\\', если есть) состоит ТОЛЬКО из одной
                # или нескольких букв, каждая с точкой (без пробелов между
                # ними) — это инициалы, не серия.
                #
                # ВАЖНО: просто обнулить series_from_block и дать функции
                # искать дальше безопасно ТОЛЬКО когда есть metadata_series,
                # способная подтвердить/направить дальнейший поиск (как в
                # случаях выше — metadata реально указывает верную серию).
                # Без metadata_series дальнейшие, менее надёжные правила
                # (Rule 3/3B и т.п.) сами способны угадать ДРУГОЙ, тоже
                # ложный кандидат — реальный случай: "Хьюитт Дж. М. -
                # Идеальная деревня" (2 самостоятельные книги без единой
                # серии, metadata_series пуста) после отбрасывания "М."
                # проваливался в Rule 3B и получал "Хьюитт Дж" — тоже
                # фрагмент имени автора, просто НЕ пойманный отдельной
                # проверкой _is_author_surname (там разбор по ОДНОМУ
                # слову). Без metadata — надёжнее вернуть "серии нет"
                # сразу, чем идти на очередной необоснованный угад.
                if series_from_block and re.match(r'^(?:[А-ЯЁA-Z]\.){1,}\s*(?:\\|$)', series_from_block):
