    ...
    def _extract_series_from_filename(self, file_path: str, validate: bool = True, metadata_series: str = "", known_series: str = "", proposed_author: str = "") -> str:
        ...
        try:
            ...
            if file_patterns:
                ...
                # Корень серии — голая инициаль ("С.", "М.") — никогда не бывает
                # настоящим названием серии. Реальный случай: "Битон М. С. -
                # Хэмиш Макбет 1. Смерть сплетницы" — двусоставный псевдоним автора
                # "М. С." (M.C. Beaton) сматчился по паттерну "Author. Series.
                # SubSeries. Title" как Author="Битон М." + Series="С." +
                # SubSeries="Хэмиш Макбет 1" — второй инициал автора ошибочно
                # принят за отдельный блок "Series". Отбрасываем ТАКОЙ
                # series_from_block целиком: если корень (до '\\', если есть)
                # состоит из одной буквы с точкой — это не серия.
                #
                ...
                if series_from_block and re.match(r'^[А-ЯЁA-Z]\.\s*(?:\\|$)', series_from_block):
