...
def parse_author_from_folder_name(folder_name: str,
    ...
    # Папка из ровно одной буквы — алфавитный указатель ("Ю", "С", "А"…),
    # никогда не настоящее имя автора, даже если эта буква сама по себе
    # совпадает с редким именем в словаре (напр. "Ю" — есть в male_names).
    # Реальный случай (docs/quality-roadmap.md, баг №49): "Азбука Социальной
    # Фантастики (833)\Ю\Юдин Борис Петрович\*.fb2" — папка "Ю" кэшировалась
    # как автор с низкой уверенностью, из-за чего РЕАЛЬНАЯ папка автора
    # "Юдин Борис Петрович" считалась "конфликтующей с родителем" и
    # ошибочно принималась за подсерию вместо автора.
