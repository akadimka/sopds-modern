...
class BookRecord:
    ...
    # Баг №109 (продолжение): стабильное имя АВТОРСКОЙ папки для всей
    # folder_dataset-группы — не зависит от того, у КАКОГО конкретно
    # файла метаданные подтверждают соавторство. `proposed_author`
    # расширяется до полного списка соавторов ("Иванов, Петров") только
    # для файлов, чьи СОБСТВЕННЫЕ метаданные это подтверждают (см. PASS4
    # "Widened N folder_dataset authors...") — у файлов с неполными
    # метаданными (напр. издатель не указал второго автора) он остаётся
    # узким. Раньше это расхождение внутри ОДНОЙ и той же серии раскидывало
    # её файлы по ДВУМ разным папкам автора в библиотеке. author_folder_root
    # хранит выигравший консенсус для ВСЕХ записей группы одинаково —
    # используется только для физического пути (synchronization.py), не
...
class Pass1ReadFiles:
    ...
    def execute(self) -> List[BookRecord]:
        ...
        # concurrent.futures.as_completed() отдаёт futures в порядке ЗАВЕРШЕНИЯ
        # воркеров, а не в порядке отправки задач — это зависит от планировщика
        # ОС и от прогона к прогону меняется. Последующие пассы (особенно
        # консенсус в Pass4) при выборе между равнозначными вариантами иногда
        # опираются на порядок records (например, "первое встреченное значение
        # побеждает"), из-за чего один и тот же код на одной и той же
        # библиотеке иногда давал разные (хоть и одинаково допустимые)
        # результаты — порядок слов в имени автора, дробные номера подсерий
        # и т.п. Сортируем по пути файла — детерминированный, не зависящий от
        # платформы порядок, восстанавливающий воспроизводимость.
        records.sort(key=lambda r: str(r.file_path))
