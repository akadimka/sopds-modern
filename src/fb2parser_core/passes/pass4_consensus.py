    ...
    def execute(self, records: List) -> None:
        ...
        for folder, group_records in groups.items():
            ...
            def _has_common_token(authors):
                # Требуется 2+ РАЗНЫХ реальных автора — «общий токен» одного автора
                # с самим собой тривиально не пуст и ничего не доказывает. Без этой
                # проверки папка с одним настоящим автором + файлом-антологией
                # (proposed_author='Сборник', исключён из authors) никогда не считалась
                # бы издательским импринтом — guard всегда возвращал бы True.
                if len(authors) < 2:
                    return False
                token_sets = [
                    {t.lower().replace('ё', 'е')
        ...
        for author, grp in _auth_groups.items():
            ...
            for rec in grp:
                ...
                # Диапазон томов текущей записи ("1-10" из имени файла) — если он
                # шире того, что покрывает "full_series" полностью самостоятельно
                # (т.е. это, судя по всему, отдельная компиляция ВСЕЙ своей
                # собственной серии), совпадение хвостового слова с ДРУГОЙ,
                # чужой серией того же автора почти наверняка случайное — см. guard
                # ниже через _nfc_lower_yo(stem).
                stem = _nfc_lower_yo(Path(rec.file_path).stem)
                for full in full_series:
                    ...
                    if full_lower.endswith(cur_lower) and full_lower != cur_lower:
                        # cur — суффикс full ТОЛЬКО текстово ("времена" — хвостовое
                        # слово "тёмные времена") не доказывает обрезанный префикс:
                        # найден реальный случай, где cur — САМОСТОЯТЕЛЬНАЯ, не
                        # связанная серия ("Времена", 10 книг), просто разделяющая
                        # последнее слово с СОВСЕМ другой серией того же автора
                        # ("Тёмные времена", 3 книги) — "Времена. Книги 1-10.fb2"
                        # ошибочно "исправлялся" в "Тёмные времена", из-за чего
                        # реальные 3 книги "Тёмные времена" считались бы покрытыми
                        # этим 10-томным файлом и удалялись бы как дубликаты.
                        # Требуем подтверждение: ПОЛНОЕ имя серии (full) должно
                        # реально встречаться в имени файла записи — как в
                        # исходном примере "Я - Миха 1. Дикарь" содержит "я - миха"
                        # целиком. Простое совпадение хвостового слова — нет.
                        if full_lower not in stem:
                            continue
        ...
        for _series_key, _recs in _series_author_groups.items():
            ...
            elif len({frozenset(_variants[a]) for a in _author_counts}) == 1:
                # Баг №109 (продолжение): у ВСЕХ вариантов не просто
                # одинаковое ЧИСЛО токенов, а буквально ОДИН И ТОТ ЖЕ набор
                # токенов — это ОДИН человек, записанный в разном порядке
                # слов ("Роман Суржиков" vs "Суржиков Роман"), а не разные
                # соавторы, случайно разделяющие общее имя (ветка ниже —
                # баг №72 — предполагает именно ЭТОТ, другой случай:
                # "Барчук Павел"/"Ларин Павел" делят токен "Павел", но их
                # ПОЛНЫЕ наборы токенов различны). Реальный случай: "Роман
                # Суржиков - Сборник произведений\Полари" — часть файлов
                # (folder_dataset) дают "Роман Суржиков, Суржиков Роман" —
                # naive comma-join ниже склеил бы обе формы ОДНОГО автора
                # как если бы это были два соавтора. Просто берём самый
                # частый вариант — сливать/объединять токены не с чем.
