...
class RegenCSVService:
    ...
    def _extract_series_from_folder_name(self, folder_name: str,
        ...
        # ШАГ 0.55: "Root N (AltName)" — N перед скобкой это номер арки/
        # позиции внутри более крупного цикла (Root), а не разделитель
        # "Серия (Автор)" (тот случай обрабатывается ниже, ШАГ 1/ШАГ 2).
        # Реальный случай (docs/quality-roadmap.md, баг №109): "Хранитель
        # 2 (Защитник тьмы)" — арка №2 цикла "Хранитель" под спин-офф-
        # названием "Защитник тьмы"; все файлы физически называются
        # "Защитник тьмы N...", а не "Хранитель 2 N...". Слепое
        # отбрасывание "(Защитник тьмы)", как если бы это было имя
        # автора, разваливало пару файлов на не связанную запись
        # "Хранитель 2" — без общего якоря для коррекции номера тома по
        # имени файла и без группировки в компиляции.
        # Отличаем от настоящего "Серия (Автор)" с числом в названии
        # серии ("Война 2020 (Марчук Николай)") тем же признаком "год vs
        # позиция", уже используемым в fb2_compiler.py (arc-эвристики):
        # короткий (1-2 цифры) хвостовой номер — позиция/арка,
        # четырёхзначный год ("2020") под этот шаблон не подходит вовсе.
        # Проверяется РАНЬШЕ паттернов ШАГ 1 ("Series (Author)" тоже
        # матчит эту форму и иначе перехватил бы её первым).
        _arc_paren_m = re.match(r'^(.+\s\d{1,2})\s*\(([^)]+)\)\s*$', folder_name)
        if _arc_paren_m and not any(c.isdigit() for c in _arc_paren_m.group(2)):
    ...
    def _postcheck_build_subfolder_hierarchy(self) -> None:
        ...
        # Баг №109 (пункт 1): подпапка-подсерия должна иметь СОБСТВЕННЫЙ
        # порядковый маркер (ведущее число "1."/"2)" или кириллическую
        # букву-порядковую "А."/"Б.") — только тогда её локальная
        # нумерация книг однозначно принадлежит общему пространству
        # позиций корня (это настоящая ДУГА серии, ср. `_sort_key_for_
        # subseries()` в fb2_compiler.py, где та же буквенная нумерация
        # уже распознаётся). Без такого маркера подпапка — по факту
        # САМОСТОЯТЕЛЬНАЯ серия со своей ГЛОБАЛЬНОЙ нумерацией книг
        # (1..N), просто физически лежащая рядом с другими сериями
        # автора в организационной папке («Мир Вальдиры», «Сборник» и
        # т.п.), а не часть одного произведения. Реальный случай: «Мир
        # Вальдиры\Герой крайних рубежей» (ГКР-1..9, своя нумерация с 1)
        # и «Мир Вальдиры\Цикл Люца» (тоже книга №1) — обе БЕЗ ведущего
        # номера папки — склейка общего корня сводила их в ОДНУ группу
        # компиляции с конфликтующими позициями томов (баг №109,
        # duplicate-дедуп это только скрывал, не решал). Контрпример:
        # «Мир Астероид-Сити\1. Нортис Вертинский» — подпапка С ведущим
        # номером, это настоящая дуга, общий корень для неё корректен.
        _ARC_ORDINAL_RE = re.compile(r'^(?:\d{1,3}\s*[.\)\-]\s?|[А-ЯЁ]\.\s)')
