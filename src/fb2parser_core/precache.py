...
class Precache:
    ...
    def execute(self, filter_paths=None) -> Dict[Path, Tuple[str, str]]:
        ...
        def scan_folder_hierarchy(folder: Path, depth: int = 0,
            ...
            # Баг №109 ("Причина A"): символьный класс выше допускает и
            # ЗАГЛАВНЫЕ буквы в "хвосте" слова, поэтому аббревиатура серии
            # в скобках ("Кроу (КРОУ)", "...(ГКН)", "...(СВА)") ложно
            # проходила ту же проверку, что и настоящая Title-Case фамилия
            # ("Широков"). Различающий признак: у фамилии в скобках хвост
            # ВСЕГДА строчный (Title Case), у аббревиатуры — весь текст
            # заглавный. Если во всём захваченном тексте нет ни одной
            # строчной буквы — это аббревиатура, не фамилия.
            if _paren_surname and _paren_surname.group(1).isupper():
                _paren_surname = None
