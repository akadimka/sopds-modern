...
class SynchronizationService:
    ...
    def _resolve_target_collision(self, source_file: Path, target_file: Path) -> bool:
        """Источник, чьё вычисленное итоговое имя совпало с уже существующим
        файлом в библиотеке, почти наверняка более новая/другая копия того же
        самого — но точного совпадения имени недостаточно, чтобы вслепую
        доверять источнику: могло быть другое издание с тем же числом томов.
        Сравниваем размеры:
          - источник НЕ БОЛЬШЕ существующего → считаем его избыточным дублем,
            удаляем источник, существующий файл в библиотеке не трогаем.
          - источник БОЛЬШЕ (потенциально более полная версия) → удаляем
            старый файл в библиотеке, возвращаем True — вызывающий код
            проваливается в обычную логику перемещения и кладёт источник на
            освободившееся место (с обычным патчем тегов).
        Тот же принцип осторожности (полнота важнее слепого доверия одной
    ...
    def _move_files(
        ...
        for i, record in enumerate(records):
            ...
            try:
                ...
                #
                # Если источник БОЛЬШЕ уже лежащего файла (потенциально более
                # полная версия) — заменяем старый файл источником: удаляем
