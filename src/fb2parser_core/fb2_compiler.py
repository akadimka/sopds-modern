...
class FB2CompilerService:
    ...
    _STEM_NUM_RE = re.compile(
        ...
        r'|\s+(\d{1,4})\s*[.\-–—_)]\s*'         # внутри: " 3. " / " 06.Название"
    ...
    # Слова-маркеры «это целая серия целиком, в одном файле» без числа
    # томов (в отличие от _SERIES_WORDS выше, где слово само задаёт
    # число). Отдельный, узкий список для _precompiled_range() — НЕ
    # config's collection_keywords, тот используется в regen_csv для
    # другого смысла ("сборник РАЗНЫХ произведений").
    _WHOLE_SERIES_MARKERS = ('цикл', 'сага')
    ...
    def _series_suffix(cls, n_volumes: int, lo: int, hi: int = None, part_count: int = 0,
        ...
        # lo==hi==0 при n_volumes>1 — это НЕ настоящий диапазон позиций
        # 0..0, а признак того, что НИ У ОДНОЙ книги нет реальной позиции
        # в серии (_run_stats считает каждую такую книгу отдельным
        # "виртуальным" томом через уникальный отрицательный id — см. её
        # комментарий). Реальный случай (docs/quality-roadmap.md, баг
        # №45): папка "Рассказы" (Белаш Александр) — 7 самостоятельных
        # рассказов без номеров тома вообще — получала суффикс
        # "Гепталогия", хотя рассказы никак не связаны последовательностью
        # (в отличие от настоящей 7-томной серии, начинающейся с 1). Для
        # НАСТОЯЩЕГО диапазона lo=hi=0 означало бы единственный "том 0"
        # (n_volumes==1) — здесь n_volumes>1 однозначно выдаёт подделку.
        _fake_zero_range = (lo == 0 and hi == 0 and n_volumes > 1)
        if _fake_zero_range:
            # Нет ни слова "N-логия" (нет реальной последовательности), ни
            # диапазона "т. N-M" (нет реальных номеров вовсе) — только
            # честный счёт книг.
    ...
    def find_groups(
        ...
        def _punct_norm(s: str) -> str:
            ...
            Баг №67 (docs/quality-roadmap.md): хвостовая пометка-уточнение
            «(Автор)»/«(СИ)» и т.п. в конце серии тоже отбрасываем перед
            нормализацией — та же конвенция, что и `_strip_author_suffix()`
            в pass4_consensus.py, только здесь она нужна ключу группировки
            find_groups(), а не отображаемому значению серии. Реальный
            случай: Клеванский Кирилл / "Сердце Дракона" — часть файлов
            несёт в метаданных "Сердце дракона (Клеванский)" (автор
            продублировал своё имя в конце серии), остальные — голое
            "Сердце Дракона"/"Сердце дракона". Без этой обрезки суффикс
            делал ключ группировки РАЗНЫМ, раскалывая одну серию на два
            несвязанных бакета компиляции.
            """
        ...
        #   + полное content_hash-совпадение книг малой группы с книгами
        #   большой (баг №109: раньше вместо хэша достаточно было числового
        #   вхождения диапазона томов малой в диапазон большой — ложно
        #   срабатывало на двух НЕЗАВИСИМЫХ подсериях одной вселенной с
        #   совпадающей по случайности локальной нумерацией, см. коммент
        #   у самой проверки ниже).
        #   Пример: Рубеж\Сирийский рубеж (т. 5-8) ⊂ Рубеж (1-11) — те же
        #   книги, поэтому хэши совпадают и без отдельной проверки диапазона.
        #
            ...
            for _author_groups in _by_author.values():
                ...
                for _small in _author_groups:
                    ...
                    for _large in _author_groups:
                        ...
                        # Реальный баг (Test1, Эльтеррус Иар): "Русский Сонм\
                        # Горькие травы" (т. 1-3, книги "Лунное стекло"/
                        # "Священный метод"/"Дар") и родитель "Русский Сонм"
                        # (т. 1-3, СОВЕРШЕННО ДРУГИЕ книги "Огонь и ветер"/
                        # "Игры морока"/"Демиурги. Полигон богов") — два
                        # НЕЗАВИСИМЫХ произведения одной вселенной автора,
                        # каждое со своей локальной нумерацией 1-3. Числовое
                        # совпадение диапазонов — совпадение, не признак
                        # общего контента. Раньше диапазон-в-диапазоне сам по
                        # себе (без проверки хэша) считался достаточным
                        # доказательством — помечал ВСЕ 3 файла "Горькие
                        # травы" как дубликаты с kept_paths=[] (ничего не
                        # остаётся!) — при применении удалило бы книги
                        # безвозвратно, не заменяя их ничем.
                        #
                        # Пример из комментария выше ("Рубеж\Сирийский рубеж
                        # (т. 5-8) ⊂ Рубеж (1-11)") остаётся рабочим: там
                        # подсерия и родитель РЕАЛЬНО делят те же файлы, так
                        # что хэши совпадут и без отдельной проверки диапазона
                        # — сам диапазон при этом никакой дополнительной
                        # гарантии не даёт, только хэш подтверждает, что речь
                        # об одном и том же контенте.
    ...
    def _extract_inline_volume_number(cls, title: str, stem: str) -> Optional[int]:
        ...
        for text in (title, stem):
            ...
            if m:
                # Баг №109: слово-ключ ("часть", "книга" и т.п.) иногда —
                # часть САМОГО названия книги ("Секретная часть"), а не
                # структурный маркер тома; если после него в тексте
                # (не обязательно сразу — паттерн допускает любой текст
                # между 1-4 цифрами) стоит год издания ("...часть -
                # 2023.fb2"), regex ловил его как "номер тома 2023".
                # Год всегда ≥1900; ни у одной реальной серии не бывает
                # 500+ томов/частей — та же граница, что уже используется
                # для арифметического варианта чуть выше и для отдельной
                # проверки "0 < book_n < 1900" в _determine_sort_key().
                n = int(m.group(1))
                if n <= 500:
                    return n
    ...
    def _precompiled_range(
                ...
                if not stem_leading:
                    ...
                    if _slink_pos >= 0:
                        ...
                        if _arc_n_m:
                            ...
                            # Баг №109 (продолжение): если у книги уже есть
                            # собственная надёжная позиция (просто число, не
                            # диапазон) из series_number, СОВПАДАЮЩАЯ с этим
                            # угаданным "arc"-номером — это обычный, простой том
                            # плоской серии на позиции N ("Демон 3. Проводник
                            # хаоса.fb2", series_number="3"), а не арка внутри
                            # франшизы с внутренней поднумерацией (мотивирующий
                            # случай — "Брия 1. Книга Длинного Солнца 1-2", где
                            # после "1." идёт ЕЩЁ один, внутренний диапазон
                            # "1-2" — признак настоящей арки, найдётся кодом
                            # ниже независимо от этого шортката). Без этой
                            # проверки find_groups() видел "Демон 3."/"Демон 4."
                            # каждую как уже готовую "предкомпиляцию" своей же
                            # позиции — из-за чего вся связка (включая настоящий
                            # омнибус "Демон. Трилогия", 1-3) считалась набором
                            # независимых arc-подсерий и полностью пропускалась
                            # find_groups() (см. "_all_precompiled"/"_any_multi"
                            # выше по файлу) — серия "Демон" не компилировалась
                            # вовсе, даже её готовая часть не отображалась.
                            _own_sn_for_arc = (
                                getattr(book.record, 'series_number', '') or ''
                            ).strip()
                            # Arc-номер не должен быть частью названия серии
                            if (_own_sn_for_arc != _arc_n_val and _arc_n_int < 1900 and
        ...
        # Критерий 1.7: слово-маркер «это целая серия одним файлом» (не
        # collection_keywords из config — тот список используется в
        # regen_csv-проходах для другого смысла, "сборник РАЗНЫХ вещей";
        # здесь нужен узкий, локальный для компилятора список маркеров
        # "весь известный цикл в одном файле") БЕЗ какого-либо числа рядом
        # вообще — трактуем как компиляцию ВСЕЙ серии (1..max_known_position),
        # если такой ориентир известен. Баг №70 (docs/quality-roadmap.md).
        # Реальный случай (Далин Макс / "Город Внизу"): "Далин - Цикл
        # «Город внизу» (СИ).fb2" — уже готовый омнибус, физически
        # содержащий все 5 повестей серии (313 КБ ≈ сумма 5 отдельных
        # файлов), но слово "Цикл" не задаёт число томов (в отличие от
        # "Трилогия"/"Пенталогия" в `_SERIES_WORDS`) — файл не проходил НИ
        # ОДИН из числовых критериев выше и полностью выпадал из
        # find_groups(): не член группы, не дубликат, просто молча
        # остаётся на диске нетронутым. Как и в критерии 1.65, без
        # `max_known_position` диапазон не строим — деградация безопасна.
        # Баг №109 (продолжение): книга с уже известной, надёжной СОБСТВЕННОЙ
        # позицией (напр. "2" из ведущей цифры в имени файла) не может ОДНОВРЕМЕННО
        # быть "всей серией в одном файле" — настоящий омнибус (баг №70, "Далин -
        # Цикл «Город Внизу»") получает позицию ТОЛЬКО через этот критерий, у него
        # нет своего номера. Реальный случай: "2. Улан. Наследие предков.fb2" —
        # обычный, единственный том (5.3 МБ, как и соседи) с ИСПОРЧЕННЫМ, похоже
        # скрейпленным с сайта, `file_title` — фраза "военная САГА" в конце
        # промо-текста (жанровый тег, не связан со структурой файла) ложно матчила
        # _WHOLE_SERIES_MARKERS, том считался "уже готовой компиляцией 1-4", а тома
        # 1/3/4 — дубликатами на удаление. Реальная потеря данных при выполнении.
        _own_sn = (getattr(book.record, 'series_number', '') or '').strip()
        _has_own_definite_position = bool(re.match(r'^\d+$', _own_sn))
        if max_known_position and max_known_position > 1 and not _has_own_definite_position:
            for candidate in (_stem_val, book.record.file_title or ''):
                if _has_series_link(candidate):
                    _cand_lower = candidate.lower()
                    if any(kw in _cand_lower for kw in self._WHOLE_SERIES_MARKERS):
                        return 1, max_known_position
    ...
    def _annotation_toc_range(self, text: str, series: str) -> Tuple[int, int]:
        ...
        #
        # Реальный случай (Каменистый Артём / "Девятый"): подпись — "Содержание
        # цикла:", а не голое "Содержание:" — точное совпадение строки не
        # срабатывало, заголовок принимался за настоящий разделитель серии,
        # слово "девятый" среди заголовков не находилось (сам заголовок ведь
        # "содержание цикла", не название серии) → весь список 1-6 ошибочно
        # отбрасывался как "чужой сборник". Допускаем одно хвостовое общее
        # уточняющее слово после "содержание"/"оглавление".
        _TOC_LABEL_RE = re.compile(
            r'^(содержание|оглавление|contents)'
            r'(?:\s+(?:цикла|серии|романа|книги|сборника|произведени[ий]))?'
            r'\s*$',
            re.IGNORECASE,
        )
        headers = [
    ...
    def _dedup_by_content(
        ...
        def _specificity(b: CompilationBook) -> int:
            sk = b.sort_key
            # sk[0]==0 — единственный уровень, где sk[1:] несёт РЕАЛЬНУЮ позицию в
            # серии (см. _determine_sort_key). На других уровнях sk[1] — год
            # ('title_date'/'publish_date', sk[0]==2/3) или вообще не задан
            # (ambiguous, sk[0]==9) — это не позиция и не должно давать
            # приоритет: год издания 2013 в sk[1] иначе ошибочно выигрывал бы у
            # книги без определённой позиции, хотя оба файла одинаково не
            # имеют доказанного места в серии.
            if sk[0] != 0:
                return 0
                    ...
                    if fn_m:
                        ...
                        if _fn_num_chk == meta_num and not _is_leading_prefix:
                            ...
                            if _kw_after:
                                ...
                                # Баг №109: то же самое, что и в _extract_inline_
                                # volume_number() — ключевое слово (том/часть/
                                # книга…) может быть частью САМОГО названия, а не
                                # структурным маркером; число после него в этом
                                # случае может оказаться годом издания. Год
                                # всегда ≥1900, ни у одной серии не бывает 500+
                                # томов — та же граница, что и везде в этом файле.
                                ...
                                if _kw_n != meta_num and _kw_n <= 500:
    ...
    def _sort_key_for_subseries(
        ...
        # Баг №107: ни корень («Мир N»), ни имя подсерии («Герой крайних
        # рубежей N») не нашли позицию в стеме/заголовке — типичный случай,
        # когда файлы называются аббревиатурой подсерии («ГКР-N.», а не
        # «Герой крайних рубежей N»). `inline` в этой ситуации мог поймать
        # ложный сигнал: «Часть N»/«Том N» в заголовке — это подзаголовок
        # КОНКРЕТНОЙ позиции («Аньгора. Часть 2» при series_number=6), а не
        # номер внутреннего тома этой позиции — 2 разных файла «Часть 1»/
        # «Часть 2» получают РАЗНЫЕ series_number (5 и 6), а не общий N с
        # разными «частями» N.1/N.2. `sn` (series_number из метаданных) уже
        # прошёл проверку согласованности с metadata_series выше по
        # конвейеру (pass1/pass2/consensus) и надёжен как прямая позиция —
        # используем его, отбрасывая ложный `inline`, когда больше ничего
        # не нашлось. Реальный случай: docs/quality-roadmap.md, баг №107.
        if not parent_num and not sub_ordinal and sn and re.match(r'^\d+$', sn):
            _sn_num = int(sn)
            if _sn_num < 1900:
                parent_num = _sn_num
                inline = 0
    ...
    def compute_group_suffix(self, group: 'CompilationGroup') -> Tuple[str, int, int]:
        ...
        if _has_exclusions:
            ...
            # в одну книгу серии.
            #
            # ИСКЛЮЧЕНИЕ: если серия неполная (_sc_compile=False — есть
            # разрыв в нумерации дуг, напр. дуги 1-3 присутствуют, а 4-я
            # отсутствует и есть более поздняя 5-я отдельно) — словесная
            # форма ("Трилогия") маскировала бы, КАКИЕ именно дуги внутри
            # этого файла: "Иванович Юрий / Миры Доставки" — дуга 5 сама по
            # себе честно получает "ч. 5 в 4 книгах" (см. ветку с
            # n_top_arcs==1 ниже), а слитые дуги 1-3 должны явно показывать
            # диапазон ("ч. 1-3 в 9 книгах"), а не выглядеть завершённой
            # серией целиком.
            #
            # Если разрыва нет (серия полностью завершена на этом run'е) —
            # словесная форма безопасна и даёт больше информации, чем голое
            # "в N книгах": число дуг (n_top_arcs) читается напрямую из
            # структуры серии (напр. "Второй Апокалипсис 1"/"Второй
            # Апокалипсис 2" — сама нумерация подсерий уже говорит "2 дуги",
            # реальный случай — Бэккер Ричард / "Второй Апокалипсис", 2
            # дуги по 3+4 сырых книги → "Дилогия в 7 книгах" вместо голого
            # "в 7 книгах").
            if not _sc_compile:
                suffix = f'ч. {top_lo}-{top_hi} в {len(group.books)} книгах'
            else:
                suffix = self._series_suffix(n_top_arcs, top_lo, top_hi,
                                             len(group.books), use_parts=True,
