...
class FB2CompilerService:
    ...
    def compute_group_suffix(self, group: 'CompilationGroup') -> Tuple[str, int, int]:
        ...
        if _has_exclusions:
            ...
            # отсутствует и есть более поздняя 5-я отдельно) — просто "в N
            # книгах" маскирует, КАКИЕ именно дуги внутри этого файла:
            # "Иванович Юрий / Миры Доставки" — дуга 5 сама по себе честно
            # получает "ч. 5 в 4 книгах" (см. ветку с n_top_arcs==1 ниже),
            # а слитые дуги 1-3 давали голое "в 9 книгах" — не видно, что
            # это именно 1-3, а не вся серия целиком. Если бы разрыва не
            # было (серия полностью завершена на этом run'е), диапазон дуг
            # не несёт новой информации — оставляем просто "в N книгах".
            if not _sc_compile:
