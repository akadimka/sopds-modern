...
urlpatterns = [
    ...
    re_path(
        r"^authors/(?P<lang_code>[0-9])/$", feeds.AuthorsFeed(), name="char_authors"
    ),
    re_path(
    ...
    re_path(
        r"^search/books/(?P<searchtype>as)/(?P<searchterms>.+)/(?P<searchterms0>.+)/(?P<page>\d+)/$",
        feeds.SearchBooksFeed(),
        name="searchbooks",
    ),
    re_path(
    ...
    re_path(r"^progress/(?P<book_id>[0-9]+)/$", dl.SaveProgress, name="save_progress"),
    re_path(
