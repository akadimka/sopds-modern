...
def _run_sync_thread():
    ...
    try:
        ...
        if auto_compile and files_moved > 0:
            # Компилируем ПОСЛЕ перемещения в библиотеку и ВСЕГДА по всей
            # библиотеке (а не только по свежесинхронизированной папке) — так
            # find_groups() видит все тома серии разом, в т.ч. одиночные тома,
            # лежавшие в библиотеке ещё до этой синхронизации, и корректно
            # решает, завершена ли серия (см. series_complete в fb2_compiler.py).
            #
            ...
            # компиляции, которая сканирует библиотеку целиком. К моменту,
            # когда этот проход по всей библиотеке успевал бы это заметить,
            # тома уже были неверно схлопнуты в один файл прошедшим проходом.
