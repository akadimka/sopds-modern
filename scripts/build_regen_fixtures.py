...
SOURCES = [
    ...
    # Вся папка (не точечные файлы) — баг №54 чувствителен к консенсусу
    # среди ВСЕХ файлов автора (proposed_series для "Округ Форд 4.
    # Рассказы" резолвится по-другому при разном наборе "соседей").
    (r"Гришэм Джон - Сборник", None, False),
    # Баг №56 — многоавторская папка-импринт ("Клуб убийств") без serier
    # metadata rescue должен уступать filename rescue при консенсусе внутри
    # ОДНОГО автора (не откатываться сразу на metadata_series).
    (r"Серия - «Коллекция МИФ»\Клуб убийств", None, False),
    # Баг №56 (вторая находка) — однородная папка ОДНОГО издательского
    # импринта ("МИФ. Проза") без реальной серии: у РАЗНЫХ авторов в ней
    # одинаковый (ложный, издательский) metadata_series ("МИФ Проза").
    # FILENAME RESCUE не должен путать эхо этого metadata_series с реальным
    # консенсусом из имени файла.
    (r"Серия - «Коллекция МИФ»\МИФ. Проза", None, False),
    # Баг №58 — "Author, Arc - Сборник произведений": запятая без скобок
    # ошибочно распознавалась как "два автора через запятую", хотя текст
    # после запятой — имя вселенной/цикла + служебное слово коллекции.
    (r"Владимир Малый, Тёмные Окна - Сборник произведений", None, False),
    # Баг №59 — "С.К.С." (инициалы-псевдоним автора) ошибочно матчился как
    # корень серии для "Режим бога"; нужен весь родительский "Вязовский
    # Алексей - Сборник" целиком, чтобы автор корректно унифицировался через
    # folder_dataset на все 12 файлов (иначе author-консенсус в изоляции
    # даёт другой, испорченный результат — не то, что видно в реальной CSV).
    (r"Вязовский Алексей - Сборник", None, True),
    # Баг №61 — "Ильф, Петров. Том N.fb2" (фамилии ДВУХ авторов через запятую
    # в имени файла) ошибочно матчилось как название серии.
    (r"Илья Ильф, Евгений Петров", None, False),
    # Баг №106 — "Мир Астероид-Сити" ложно считался вариантной папкой (СИ/
    # самиздат-маркер как подстрока внутри "Сити"), терялся весь корневой
    # уровень серии + номер подсерии. Две подсерии ("1. Нортис Вертинский",
    # "2. Тимофей Градский") нужны обе — проверяют, что номер подсерии
    # сохраняется корректно для каждой из них.
    (r"Михайлов Руслан - Сборник\Мир Астероид-Сити", None, True),
    # Баг №107 — компилятор (fb2_compiler.py) разбивал "Мир Вальдиры\Герой
    # крайних рубежей" (ГКР-1..9, аббревиатура без полного имени подсерии
    # в стеме) на два несвязанных диапазона (0-4 и 7-9), теряя ГКР-5
    # целиком и превращая ГКР-6 в позицию "0.2" — "Часть 2" в заголовке
    # ложно принималось за номер тома внутри позиции. "Цикл Люца" — минимальный
    # сосед с ДРУГИМ именем подсерии в том же корне "Мир Вальдиры", нужен
    # чтобы регрессия проявилась (при единственной подсерии в корне баг
    # маскируется побочным эффектом другого прохода, см. quality-roadmap.md).
    (r"Михайлов Руслан - Сборник\Мир Вальдиры\Герой крайних рубежей", None, False),
    (r"Михайлов Руслан - Сборник\Мир Вальдиры\Цикл Люца", None, False),
    # Баг №109 (продолжение) — "Киндрэт - Пехов,Бычкова, Турчанинова"
    # (формат "Серия - Автор1,Автор2, Автор3", серия ПЕРЕД тире) ложно
    # давал proposed_series = список авторов ПОСЛЕ тире — существующий
    # фикс бага №59 предполагал только обратный порядок ("Авторы -
    # Серия") и брал текст после тире безусловно, не проверяя, что там
    # на самом деле список авторов.
    (r"Пехов Алексей - Сборник\Киндрэт - Пехов,Бычкова, Турчанинова", None, False),
    # Баг №109 (продолжение) — "Начинается вьюга.fb2" лежит ПРЯМО в корневой
    # папке автора (не в подпапке серии), т.е. без ЛЮБОГО папочного сигнала
    # о серии, хотя metadata_series="Хроники Сиалы" (существует другая
    # реальная серия/подпапка с этим именем в другом месте библиотеки).
    # METADATA RESCUE (pass4_consensus.py) ошибочно присваивал ей эту серию
    # из голых метаданных — файл должен остаться БЕЗ серии и лечь в корень
    # авторской папки. Только верхний уровень (не рекурсивно) — так же, как
    # эти 23 отдельных рассказа лежат в реальной библиотеке.
    (r"Пехов Алексей - Сборник", None, False),
]
