...
class TestAnnotationTableOfContentsRange:
    ...
    def test_annotation_toc_label_with_qualifier_word_recognized(self):
        """Обнаружено на реальной библиотеке (Каменистый Артём / "Девятый"):
        подпись перед списком — "Содержание ЦИКЛА:", а не голое
        "Содержание:" — точное совпадение `_TOC_LABEL_RE` не срабатывало,
        заголовок принимался за настоящий разделитель серии (как "АРТУР
        РЭЙШ:"), слово "девятый" среди заголовков не находилось (сам
        заголовок — "содержание цикла", не название серии), и весь список
        1-6 ошибочно отбрасывался как "чужой сборник, наша серия не
        найдена" вместо распознавания уже готовой Гексалогии.
        """
        text = (
            '<annotation>'
            '<p>Аннотация.</p>'
            '<p><strong>Содержание цикла:</strong></p>'
            '<p>1. Девятый.</p>'
            '<p>2. На руинах Мальрока.</p>'
            '<p>3. Рождение победителя.</p>'
            '<p>4. Адмирал Южных морей.</p>'
            '<p>5. Сердце для стража.</p>'
            '<p>6. Тайны ордена.</p>'
            '</annotation>'
        )
        svc = FB2CompilerService()
        assert svc._annotation_toc_range(text, "Девятый") == (1, 6)
